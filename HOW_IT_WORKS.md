...
5. The samplesheet is parsed and validated. `INPUT_CHECK` runs `bin/check_samplesheet.py` (stdlib-only Python, on Nextflow's PATH) which enforces the format, normalizes the rows, and writes `*.valid.csv`. Nextflow then `splitCsv`s that file into a channel of `[ meta, [fastqs] ]`, where `meta` carries `id`, `single_end`, and `status`. [M1]
   ...
   - **Report** (`REPORT`): `PRIORITIZE_VARIANTS` (`bin/prioritize_variants.py`) tiers variants by predicted impact; `GENERATE_REPORT` (`bin/generate_report.py`) writes an HTML + JSON report plus an HL7 Genomics Reporting IG-aligned FHIR R4 bundle (see `docs/FHIR.md`). Both are stdlib-only and run offline; an LLM narrative is optional. [M5, M6, H7]
